...
def run_search(easting, northing, search_id, reference):
    ...
    # invokes random account picker function
    account_details = random_account_picker()
    ...
    try:
        driver.find_element_by_xpath(
            '//*[@id="WorkBenchPanel"]/div/div[3]/div/div[2]').click()
    except Exception as e:
        print(e, 'could not find by xpath')
        driver.find_element_by_css_selector(
            '#WorkBenchPanel > div > div:nth-child(4) > div > div.BenchTool.NewEnquiry.Orange').click()
        print('found by css selector!')

    quickwait()  # waiting...
    driver.find_element_by_id('NewEnquiry.CommencesOn').send_keys(scheme_date)
    superquickwait()  # waiting...
    # script = f'document.querySelector("#NewEnquiry\\\.CompletesOn").value = "{scheme_date}";'
    # driver.execute_script(script)
    driver.find_element_by_id('NewEnquiry.CompletesOn').send_keys(scheme_date)
    superquickwait()  # waiting...
    # def sendkeys(key):
    #     for letter in key:
    #         driver.find_element_by_css_selector("#NewEnquiry\.UserReference").send_keys(letter)
    # sendkeys(search_id)
    driver.find_element_by_css_selector("#NewEnquiry\.UserReference").send_keys(search_id)
    superquickwait()  # waiting...
    scale_dropdown = Select(
        driver.find_element_by_id('NewEnquiry.PaperPlanScale'))
    scale_dropdown.select_by_visible_text(Scale)
    superquickwait()
    behalf_of_dropdown = Select(
        driver.find_element_by_id('NewEnquiry.WorkingOnBehalfOf'))
    ...
    quickwait()  # waiting...
    # def sendEasting (east):
    #     for num in east:
    #         driver.find_element_by_css_selector('#Coordinates\.East').send_keys(num)
    #         time.sleep(5)
    #         print(num)
    # sendEasting(easting)
    driver.find_element_by_css_selector('#Coordinates\.East').send_keys(easting)
    superquickwait()
    # def sendNorthing (north):
    #     for num in north:
    #         driver.find_element_by_css_selector('#Coordinates\.North').send_keys(num)
    # sendNorthing(northing)
    ...
    for index, _ in df.iterrows():
        ...
        if company != NG:
            if status == 'Email Additional Info':
                # email additional info handling
                database_writer.update_lsbud_table(
                    company=company, lsbud_ref=enquiry_id)

        if company in company_ignore_list:
            # if company found in ignore list set search_complete to false
            database_writer.update_lsbud_table_difficult_companies(
                company=company, lsbud_ref=enquiry_id)
    ...
    driver.quit()
    # updates main table with completed search
    database_writer.update_main_searches_LSBUD(search_id=search_id)


if __name__ == "__main__":
    while True:
        # row = database_writer2.query_searches_main_lsbud()  # looks for new and incomplete searches in main table
        row = database_writer.lsbud_account_1()  # looks for 0s
        if row:
            search_values = list(row)
            print(search_values)
            print('new search found')
            try:
                run_search(easting=str(search_values[7]), northing=str(search_values[8]), search_id=str(search_values[0]),
