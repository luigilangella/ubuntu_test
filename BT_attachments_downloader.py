...
MAILBOX = 'INBOX/BT'  # Mailbox to check
    ...
    for part in msg.walk():
        ...
        if bool(file_name):  # If bool(file_name) returns True
            file_path = os.path.join(save_dir,
                                     '291 ' + file_name)  # Combine the save directory and file name to make file_path
...
def process_email():
    ...
    for row in rows:
        ...
        for msgid, data in response.items():  # Iterates through the collection and assigns to 2 variables one by one
            ...
            find_ref = re.search('([A-Z0-9]{9})', subject)
            if bt_ref == find_ref.group(0):
                # input('match!')
                get_attachments(raw, save_dir=save_dir)
                database_writer.update_main_searches_bt_data_received(search_id=search_id)
                database_writer.update_bt_table(search_id=search_id)
                server.move(msgid, 'INBOX/BT/complete')
            # time.sleep(5)
...
if __name__ == '__main__':
    while True:
        rows = database_writer.query_bt_table()
        if rows:
            print(len(rows))
            try:
